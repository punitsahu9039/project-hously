...
function ChatWidget() {
  ...
  const images = [
    "./src/assets/images/chat2.jpg",
    "./src/assets/images/chat1.jpg",
  ];
  ...
  const menuOptions = isHindi
    ...
    : ["Apply for Loan", "Ask a Question", "Get Insured", "EMI Calculator", "Self-Service", "Overdue EMI Quick Pay", "Cancel Conversation", "Pre Approved Offer", "Account Details"];
  ...
  return (
    <div style={{ fontFamily: "'Glacial Indifference', sans-serif" }}>
      ...
      {isOpen && (
        <div
          className="card shadow-lg chat-widget"
          style={{
            position: "fixed",
            bottom: "90px",
            right: "20px",
        ...
        >
          ...
          >
            <div><strong>Shrey - Virtual Assistant</strong></div>
            <button className="btn-close btn-close-white" onClick={toggleChat}></button>
          </div>

          <div className="card-body text-center">
            <img src="./src/assets/images/chat1.jpg" alt="avatar" width="60" height="60" />
            ...
            {!languageSelected ? (
              ...
              <>
                  ...
                  <>
                    ...
                    <div className="d-flex flex-wrap justify-content-center gap-2">
                      {(showAllOptions ? options : options.slice(0, 4)).map((option, index) => (
                        <button key={index} className="btn btn-light option-button" style={{
                ...
                <div className="input-group mt-3">
                  ...
                  <button className="btn" onClick={handleSendMessage} style={{ backgroundColor: "#0074d9", color: "#fff" }}>
                    ➤
                  </button>
                </div>

                {showMenu && (
                  <div className="mt-3 p-3 rounded border bg-white text-start overflow-auto" style={{ maxHeight: "300px" }}>
                    {menuOptions.map((option, index) => (
                      <div key={index} className="mb-2 border-bottom pb-2">{option}</div>
                    ))}
                  </div>
                )}
              </>
